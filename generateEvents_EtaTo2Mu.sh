...
echo "Will save generated AODs at: /store/user/$USERNAME/EtaTo2Mu/AOD_Signal_Samples/"
...
cp ../../EtaTo2Mu_DIGIRAWHLT_template_2018_cfg.py .

RANDOMSEED=`od -vAn -N4 -tu4 < /dev/urandom`
#Sometimes the RANDOMSEED is too long for madgraph
RANDOMSEED=`echo $RANDOMSEED | rev | cut -c 3- | rev`

namebase="EtaTo2Mu_$RANDOMSEED"

echo "1.) Generating GEN-SIM for EtaTo2Mu from pluto events"

cmsDriver.py Configuration/Generator/python/PlutoReader_EtaTo2Mu_pythia8_cfi.py \
    ...
    --python_filename EtaTo2Mu_GENSIM_cfg.py --no_exec -n ${nevent} || exit $?;
...
linenumber=`grep -n 'process.source' EtaTo2Mu_GENSIM_cfg.py | awk '{print $1}'`
linenumber=${linenumber%:*}
total_linenumber=`cat EtaTo2Mu_GENSIM_cfg.py | wc -l`
bottom_linenumber=$(( $total_linenumber - $linenumber ))
tail -n $bottom_linenumber EtaTo2Mu_GENSIM_cfg.py > tail.py
head -n $linenumber EtaTo2Mu_GENSIM_cfg.py > head.py
...
cat tail.py >> head.py
mv head.py EtaTo2Mu_GENSIM_cfg.py
rm -rf tail.py

cmsRun -p EtaTo2Mu_GENSIM_cfg.py
...
echo "2.) Generating DIGI-RAW-HLT for EtaTo2Mu"

sed -i "s/file:placeholder_in.root/file:${namebase}_GENSIM.root/g" EtaTo2Mu_DIGIRAWHLT_template_2018_cfg.py
sed -i "s/file:placeholder_out.root/file:${namebase}_DIGIRAWHLT.root/g" EtaTo2Mu_DIGIRAWHLT_template_2018_cfg.py
sed -i "s/input = cms.untracked.int32(10)/input = cms.untracked.int32(${nevent})/g" EtaTo2Mu_DIGIRAWHLT_template_2018_cfg.py

mv EtaTo2Mu_DIGIRAWHLT_template_2018_cfg.py EtaTo2Mu_DIGIRAWHLT_cfg.py
...
#    --python_filename EtaTo2Mu_DIGIRAWHLT_cfg.py \ 
#    --customise Configuration/DataProcessing/Utils.addMonitoring \
#    --no_exec || exit $?;

cmsRun -p EtaTo2Mu_DIGIRAWHLT_cfg.py

echo "3.) Generating AOD for EtaTo2Mu"
...
cmsDriver.py step2 \
    ...
    --procModifiers premix_stage2 \
    --nThreads 1 --era Run2_2018 --python_filename EtaTo2Mu_AOD_cfg.py --no_exec \
    --customise Configuration/DataProcessing/Utils.addMonitoring -n ${nevent} || exit $?;

cmsRun -p EtaTo2Mu_AOD_cfg.py
...
xrdcp ${namebase}_AOD_2018.root root://cmseos.fnal.gov//store/user/$USERNAME/EtaTo2Mu/AOD_Signal_Samples/
